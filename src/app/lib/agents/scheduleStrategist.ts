...
function generateDeterministicSchedule(
    ...
    // Helper to get actual start time for a day (offsetting if today)
    const getEffectiveStartTime = (dateStr: string, baseStart: string) => {
        if (dateStr !== todayStr) return baseStart;
        
        const now = new Date();
        const currentH = now.getHours();
        const [baseH, baseM] = baseStart.split(':').map(Number);
        
        // If the current time is already past the base start, start from (Next Hour):00
        if (currentH >= baseH) {
            return `${String(currentH + 1).padStart(2, "0")}:00`;
        }
        return baseStart;
    };
    ...
    while (
        ...
        if (!isBlocked && isStudyDay) {
            ...
            const dailyStart = getEffectiveStartTime(currentDate, slot.start);
            ...
            while (reviewQueue.length > 0 && hoursUsedToday + 1 <= dailyHours) {
                ...
                const reviewStart = addHoursToTime(dailyStart, hoursUsedToday);
            ...
            while (
                ...
                const studyStart = addHoursToTime(dailyStart, hoursUsedToday);
...
export async function runScheduleStrategist(
    ...
    if (ollamaOk) {
        try {
            ...
            agentLog(AGENT_NAME, "Ollama call failed", e);
        }
    }
    ...
    if (!usedOllama) {
        agentLog(AGENT_NAME, "Ollama offline or parsing failed — using deterministic scheduler");
        events = generateDeterministicSchedule(input);
    }
