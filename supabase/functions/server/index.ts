...
api.get("/ai/agents/status", authMiddleware, async (c) => {
  const user = c.get("user");
  const [subjects, events, reports, xp, achievements] = await Promise.all([
    kv.get(`user:${user.id}:subjects`),
  ...
  const roadmap = Array.isArray(subjects) && subjects.length > 0 ? subjects[0] : subjects;
  return c.json({
