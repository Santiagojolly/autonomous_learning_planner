...
function calculateMasteryScores(
    ...
    // Group events by subject (ignore 'Busy' blocks in progress stats)
    for (const evt of events) {
        if (evt.sessionType === "Busy") continue;
        const s = evt.subject;
        if (!subjects[s]) {
            subjects[s] = {
                subject: s,
        ...
        if (evt.completed) subjects[s].completedEvents.push(evt);
    }
    ...
    for (const [subject, data] of Object.entries(subjects)) {
        ...
        // Determine trend from last 2 reports vs now
        const trend: MasteryScore["trend"] =
            rawScore > 70 ? "improving" : rawScore > 30 ? "stable" : "declining";
...
function generateRecommendation(
    ...
    if (streakDays === 0) {
        if (masteryScores.every(m => m.topicsCompleted === 0)) {
            return "Welcome to your new study journey! Your roadmap is ready and waiting for your first step. Let's make it a great first day! 🌱";
        }
...
async function generateAISummary(
    ...
    const prompt = `Generate a warm, encouraging 2-3 sentence weekly study progress summary.
Data: ${JSON.stringify({ masteryScores, streakDays, onTrack, behind })}

CRITICAL: If mastery across ALL subjects is 0 and streak is 0, DO NOT say the user has done "solid work". Instead, welcome them to their new journey and encourage them to complete their first session.

Keep it personal, motivating, and specific. Don't use generic phrases. Return only the summary text.`;
...
function generateFallbackSummary(
    ...
    if (streakDays === 0 && masteryScores.every(m => m.topicsCompleted === 0)) {
        return "Welcome to your new study journey! Your roadmap is ready and waiting for your first step. Let's make it a great first day! 🌱";
    }
    ...
    if (behind.length > 0 && streakDays === 0) {
        if (masteryScores.every(m => m.topicsCompleted === 0)) {
            return `Welcome to your new study journey! Your roadmap for ${masteryScores[0]?.subject ?? "your goals"} is ready and waiting. Your first session is scheduled and it's the perfect time to start. Let's build that first streak! 🌱`;
        }
    ...
    if (masteryScores.length === 0 || (avgScore === 0 && streakDays === 0)) {
        return `Welcome to your new study journey! Your learning architecture is fully calibrated and waiting for your first interaction. Take a small step today to activate your path! 🚀`;
    }
...
export async function runProgressOracle(
    ...
    const masteryScores = calculateMasteryScores(roadmap, events);
    ...
    // ── Generate AI summary ───────────────────────────────────────────────────
    const { summary: aiSummary, usedOllama } = await generateAISummary(
        masteryScores,
        streakDays,
        onTrack,
        behind
    ...
    const hasStarted = masteryScores.some(m => m.topicsCompleted > 0);
    const needsAdaptiveReschedule = (behind.length > 0 || streakDays === 0) && hasStarted;
    ...
    agentLog(AGENT_NAME, "Report generated", {
        weekNumber,
